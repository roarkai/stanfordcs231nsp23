...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ## 1. prepare inputs of transformers
        #     (1)memory: 把vision feature转变成cross attention的K和V所需维度
#         print(captions[0]) # 查看captions中element的形态
        memory = self.visual_projection(features) # shape:(N, W)
        memorys = memory.unsqueeze(1)             # shape:(N, 1, W)
        assert memorys.shape == (N, 1, memory.shape[1])
        
        #     (2)embeddings of captions
        capt_emb = self.embedding(captions)
        
        #     (3)add positional encoding to captions
        ped_capt_emb = self.positional_encoding(capt_emb)
        
        #     (4)mask
        Query_len = T
        _, Key_len, _ = memorys.shape # 注意，这里Key_len=1
        mask = torch.tril(torch.ones((Query_len, Key_len), dtype=torch.bool), diagonal=1)
#         mask = torch.tril(torch.ones((Query_len, Key_len), dtype=torch.bool))
    
        ## 2. decode to get raw scores
        scores = self.transformer(ped_capt_emb, memorys, tgt_mask=mask)
        
        ## 3. final: linear and no!!! softmax
