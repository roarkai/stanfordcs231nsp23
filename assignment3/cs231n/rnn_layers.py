...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    h = np.zeros((T, N, H))     # h has shape: (T, N, H)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    scores = x @ Wx + prev_h @ Wh + b   # scores.shape = N, 4H
    s_add, s_forget, s_out, s_tanh = np.hsplit(scores, 4)
    
    g_forget = sigmoid(s_forget)
    g_add = sigmoid(s_add)
    g_out = sigmoid(s_out)
    i_new = np.tanh(s_tanh)
    
    next_c = g_forget * prev_c + g_add * i_new
    origin_out = np.tanh(next_c)
    next_h = origin_out * g_out
    
    cache = (prev_c, prev_h, x, Wx, Wh,g_forget, g_add, g_out, i_new, origin_out)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    prev_c, prev_h, x, Wx, Wh,g_forget, g_add, g_out, i_new, origin_out = cache
    
    dnext_c += dnext_h * g_out * (1 - origin_out ** 2)
    
    dg_forget = dnext_c * prev_c
    ds_forget = dg_forget * g_forget * (1 - g_forget)
    
    dg_add = dnext_c * i_new
    ds_add = dg_add * g_add * (1 - g_add)
    
    di_new = dnext_c * g_add
    ds_tanh = di_new * (1 - i_new ** 2)
    
    dg_out = dnext_h * origin_out
    ds_out = dg_out * g_out * (1 - g_out)
    
    dscores = np.concatenate((ds_add, ds_forget, ds_out, ds_tanh), axis=1)
    dx = dscores @ Wx.T
    dprev_h = dscores @ Wh.T
    dprev_c = dnext_c * g_forget
    dWx = x.T @ dscores
    dWh = prev_h.T @ dscores
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    x = x.transpose(1, 0, 2) # 将x的shape调整为 T,N,D
    prev_c = np.zeros_like(h0)
    h = np.zeros((T, N, H))
    cache = []
    
    h[0], prev_c, cache0 = lstm_step_forward(x[0], h0, prev_c, Wx, Wh, b)
    cache.append(cache0)
    for i in range(1, T):
        h[i], prev_c, cache_i = lstm_step_forward(x[i], h[i-1], prev_c, Wx, Wh, b)
        cache.append(cache_i)
...
def lstm_backward(dh, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, T, H = dh.shape
    _, D = cache[0][2].shape # cache[i][2]是x，shape是N,D
    ...
    dh = dh.transpose(1, 0, 2) # 将dh的shape转为(T, N, H)
    ...
    for i in reversed(range(T)):
        ...
        dx[i], dprev_h, dprev_c, dWxi, dWhi, dbi = lstm_step_backward(dnext_h, dprev_c, cache[i])
        dWx += dWxi
        dWh += dWhi
        db += dbi
